venv_path := ~/.venv/adidas_django_auth
...
shell:
	@echo 'To activate the venv use source ~/.venv/adidas_django_auth/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 adidas_django_auth -r -d
	${bin_path}/autopep8 adidas_django_auth -r -i
	@echo '---Running isort---'
	${bin_path}/isort adidas_django_auth --diff
	${bin_path}/isort adidas_django_auth --atomic
